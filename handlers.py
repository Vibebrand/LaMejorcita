...
class GetPuntosHandler(cyclone.web.RequestHandler):
    ...
    def get(self):
        result = []
        for i in range(24):
            x={"_id":str(ObjectId()),"status":"Valido","phone":str(9789560+i),"email":"punto"+str(i+1)+"@gmail.com","joinDate":"15enero2010","fridge":{},"representative":{"name":"representante"+str(i+1),"phone":str(9789560+i),"email":"representante"+str(i+1)+"@gmail.com"},"stock":{"name":"Bodega"+str(i+1)}}
            result.append(x)
        self.write({"result": result})
        self.finish()
...
class GetVentasHandler(cyclone.web.RequestHandler):
    ...
    def get(self):
        result = []
        for i in range(24):
            x={"_id":str(ObjectId()),"date":"15enero2011","time":"11:59pm","observations":"observacion","amount":"4","products":{},"geoposition":{"latitude":"22.8818","longitude":"-102.2913"},"seller":{"name":"Vendedor"+str(i+1)},"salepoint":{"fridge":{}}}
            result.append(x)
        self.write({"result": result})
        self.finish()
...
class GetProductosHandler(cyclone.web.RequestHandler):
    ...
    def get(self):
        result = []
        for i in range(24):
            x={"_id":str(ObjectId()),"status":"Valido","name":"carne1","salePrice":"150","registrationDate":"15enero2011"}
            result.append(x)
        self.write({"result": result})
        self.finish()
